...
func footerView(active view, width int, version string, errShowing bool, extra ...footerBinding) string {
	...
	if errShowing {
		...
		case viewSprint:
			split := footerBinding{"v", "split"}
			bindings = []footerBinding{nav, scroll, open, back, filter, board, split, move, link, copyURL, jql, filters, create, wiki, refresh, boards, profile, setup, help}
		case viewBoard:
